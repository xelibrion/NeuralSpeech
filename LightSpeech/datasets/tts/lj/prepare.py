...
def g2p_job(idx: int, filename: str, transcription: str) -> Tuple[List[str], List[str]]:
    speaker_id = idx // 100
    phonems: List[str] = [p.replace(" ", "|") for p in g2p(transcription)]
    phs_str = " ".join(phonems)

    os.makedirs(f"{basedir}/mfa_input/{speaker_id}", exist_ok=True)

    with open(f"{basedir}/mfa_input/{speaker_id}/{filename}.ph", "w") as f_txt:
        phs_str = re.sub("([\!'\,\-\.\?])+", r"\1", phs_str)
        f_txt.write(phs_str)

    with open(f"{basedir}/mfa_input/{speaker_id}/{filename}.lab", "w") as f_txt:
        phs_str_mfa = re.sub("[\!'\,\-\.\?]+", "PUNC", phs_str)
        phs_str_mfa = re.sub("\|", "SEP", phs_str_mfa)
        f_txt.write(phs_str_mfa)

    subprocess.check_call(
        f'cp "{basedir}/wavs/{filename}.wav" "{basedir}/mfa_input/{speaker_id}/"',
        shell=True,
    )
...
if __name__ == "__main__":
    ...
    meta_ori_df = pd.read_csv(
        os.path.join(basedir, "metadata.csv"),
        delimiter="|",
        names=["wav", "txt1", "txt2"],
    )
    subprocess.check_call(
        f"mkdir -p {basedir}/mfa_input_txt; "
        f"cp {basedir}/mfa_input/*/*.ph {basedir}/mfa_input_txt",
        shell=True,
    )

    meta_ori_df["phone2"] = meta_ori_df.apply(
        lambda r: open(f"{basedir}/mfa_input_txt/{r['wav']}.ph").readlines()[0].strip(),
        1,
    )
