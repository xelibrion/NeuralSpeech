...
class LightSpeechDataset(BaseDataset):
    ...
    def __init__(
        self, data_dir, phone_encoder, prefix, hparams, shuffle=False, infer_only=False
    ):
    ...
    def _get_item(self, index):
        if not self.use_indexed_ds:
            ...
            item = np.load(
                f"{self.data_dir}/{self.prefix}/{key}.npy", allow_pickle=True
            ).item()
        else:
    ...
    def collater(self, samples):
        ...
        f0 = (
            utils.collate_1d([s["f0"] for s in samples], -200)
            if self.hparams["use_pitch_embed"]
            else None
        )
        uv = (
            utils.collate_1d([s["uv"] for s in samples])
            if self.hparams["use_pitch_embed"]
            else None
        )
        energy = (
            utils.collate_1d([s["energy"] for s in samples], pad_idx)
        ...
        prev_output_mels = utils.collate_2d(
            [s["target"] for s in samples], pad_idx, shift_right=True
        )
...
class LightSpeechTask(BaseTask):
    ...
    def train_dataloader(self):
        train_dataset = LightSpeechDataset(
            hparams["data_dir"],
            self.phone_encoder,
            hparams["train_set_name"],
            hparams,
            shuffle=True,
        )
    ...
    def val_dataloader(self):
        valid_dataset = LightSpeechDataset(
            hparams["data_dir"],
            self.phone_encoder,
            hparams["valid_set_name"],
            hparams,
            shuffle=False,
        )
        return self.build_dataloader(
    ...
    def test_dataloader(self):
        test_dataset = LightSpeechDataset(
            hparams["data_dir"],
            self.phone_encoder,
            hparams["test_set_name"],
            hparams,
            shuffle=False,
        )
        return self.build_dataloader(
            test_dataset, False, self.max_eval_tokens, self.max_eval_sentences
        )
    ...
    def build_dataloader(
        ...
        if required_batch_size_multiple == -1:
            required_batch_size_multiple = (
                torch.cuda.device_count() if torch.cuda.device_count() > 0 else 1
            )
    ...
    def _training_step(self, sample, batch_idx, _):
        ...
        loss_output, output = self.run_model(
            self.model,
            input,
            mel2ph,
            spk_embed,
    ...
    def loss(self, decoder_output, target):
        ...
        stop_loss = (
            self.weighted_cross_entropy_with_logits(
                stop_mask, predicted_stop, hparams["stop_token_weight"]
            )
            * seq_mask
        ).sum()
        stop_loss = stop_loss / (
            seq_mask.sum() + target.size(0) * (hparams["stop_token_weight"] - 1)
        )
    ...
    def test_step(self, sample, batch_idx):
        logging.info(
            "inferring batch {} with {} samples".format(batch_idx, sample["nsamples"])
        )
    ...
    def infer_batch(self, sample):
        ...
        stage = 0
        decoder_input = (
            input.new(bsz, decode_length + 1, hparams["audio_num_mel_bins"])
            .fill_(0)
            .float()
        )
        ...
        if hparams["attn_constraint"]:
            for i, layer in enumerate(model.decoder.layers):
                enc_dec_attn_constraint_mask = (
                    input.new(bsz, self.num_heads[i], max_input_len).fill_(0).int()
                )
                layer.set_buffer(
                    "enc_dec_attn_constraint_mask",
                    enc_dec_attn_constraint_mask,
        ...
        while True:
            ...
            for i in range(hparams["dec_layers"]):
                encdec_attn_logits[i] = torch.cat(
                    (encdec_attn_logits[i], attn_logits[i]), dim=2
                )
            step += 1
            ...
            if hparams["attn_constraint"]:
                ...
                all_prev_weights = []
                for i in range(hparams["dec_layers"]):
                    all_prev_weights.append(
                        torch.softmax(encdec_attn_logits[i], dim=-1)
                    )  # bsz x head x L x L_kv
                ...
                if next_stage:  # TODO
                    ...
                    for layer in model.decoder.layers:
                        layer.clear_buffer(
                            input, encoder_out, encoder_padding_mask, incremental_state
                        )

                    encdec_attn_logits = []
                    for i in range(hparams["dec_layers"]):
                        encdec_attn_logits.append(
                            input.new(bsz, self.num_heads[i], 0, max_input_len)
                            .fill_(0)
                            .float()
                        )
                    decoded_mel = (
                        input.new(bsz, 0, hparams["audio_num_mel_bins"])
                        .fill_(0)
                        .float()
                    )
                    decoder_input = (
                ...
                prev_weights_mask1 = utils.sequence_mask(
                    torch.max(
                        attn_pos - 1, attn_pos.new(attn_pos.size()).fill_(0)
                    ).float(),
                    encdec_attn_logits[0].size(-1),
                ).float()  # bsz x L_kv
                prev_weights_mask2 = (
                    1.0
                    - utils.sequence_mask(
                        attn_pos.float() + 4, encdec_attn_logits[0].size(-1)
                    ).float()
                )  # bsz x L_kv
                enc_dec_attn_constraint_masks = []
                for i in range(hparams["dec_layers"]):
                    mask = (prev_weights_mask1 + prev_weights_mask2)[
                        :, None, :
                    ] * use_masks[i][
                        None, :, None
                    ]  # bsz x head x L_kv
                ...
                def should_move_on():
                    ...
                    for i in range(hparams["dec_layers"]):
                        prev_weight = (
                            all_prev_weights[i] * use_masks[i][None, :, None, None]
                        ).sum(dim=1)
                        prev_weights.append(prev_weight)
                    prev_weights = sum(prev_weights) / sum(
                        [mask.sum() for mask in use_masks]
                    )
                    move_on = (
                        prev_weights[:, -3:]
                        .mean(dim=1)
                        .gather(1, attn_pos[:, None].long())
                    ).squeeze(-1) < 0.7
        ...
        if not hparams["profile_infer"]:
            ...
            focus_rate = utils.get_focus_rate(
                encdec_attn, src_padding_mask, target_padding_mask
            )
            phone_coverage_rate = utils.get_phone_coverage_rate(
                encdec_attn, src_padding_mask, src_seg_mask, target_padding_mask
            )
            attn_ks = src_lengths.float() / target_lengths.float()
            diagonal_focus_rate, diag_mask = utils.get_diagonal_focus_rate(
                encdec_attn,
                attn_ks,
                target_lengths,
                src_padding_mask,
    ...
    def inv_spec(self, spec, pitch=None, noise_spec=None):
        ...
        if hparams["vocoder"] == "pwg":
            wav_out = generate_wavegan(
                spec, *self.vocoder, profile=hparams["profile_infer"]
            )
            if hparams["gen_wav_denoise"]:
                noise_out = (
                    generate_wavegan(noise_spec, *self.vocoder)[None, :]
    ...
    def dur_loss(self, dur_pred, mel2ph, input, split_pause=False, sent_dur_loss=False):
        B, T_t = input.shape
        dur_gt = mel2ph.new_zeros(B, T_t + 1).scatter_add(
        ...
        if split_pause:
            is_pause = (
                (input == self.phone_encoder.seg())
                | (input == self.phone_encoder.unk())
                | (input == self.phone_encoder.eos())
            )
            is_pause = is_pause.float()
            phone_loss = (
                self.dur_loss_fn(dur_pred, dur_gt, (1 - is_pause) * nonpadding)
                * hparams["lambda_dur"]
            )
            seg_loss = (
                self.dur_loss_fn(dur_pred, dur_gt, is_pause) * hparams["lambda_dur"]
            )
            return phone_loss, seg_loss
        ph_dur_loss = (
            self.dur_loss_fn(dur_pred, dur_gt, nonpadding) * hparams["lambda_dur"]
        )
        if not sent_dur_loss:
            ...
            sent_dur_loss = F.l1_loss(
                dur_pred.sum(-1), dur_gt.sum(-1), reduction="none"
            ) / dur_gt.sum(-1)
    ...
    def pitch_loss(self, p_pred, pitch, uv):
            ...
            uv_loss = (
                (
                    F.binary_cross_entropy_with_logits(
                        p_pred[:, :, 1].reshape(-1), uv.reshape(-1), reduction="none"
                    )
                    * nonpadding
                ).sum()
                / nonpadding.sum()
        ...
        pitch_loss = (
            (
                pitch_loss_fn(
                    p_pred[:, :, 0].reshape(-1), pitch.reshape(-1), reduction="none"
                )
                * nonpadding
            ).sum()
            / nonpadding.sum()
            * hparams["lambda_pitch"]
        )
    ...
    def test_step(self, sample, batch_idx):
        ...
        if hparams["gen_wav_denoise"]:
            ...
            noise_outputs = self.model(
                input_noise, mel2ph_noise, spk_embed, None, pitch_noise, uv_noise
            )
            sample["noise_outputs"] = noise_outputs["mel_out"]

        sample["outputs"] = outputs["mel_out"]
        sample["pitch_pred"] = outputs.get("pitch")
        sample["pitch"] = restore_pitch(
            sample["pitch"], uv if hparams["use_uv"] else None, hparams
        )
        # if hparams['profile_infer']:
        #    return {}
