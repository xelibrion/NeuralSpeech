...
def get_pitch(wav_data, mel, hparams):
    ...
    f0 = pw.stonemask(
        wav_data.astype(np.double), _f0, t, hparams["audio_sample_rate"]
    )  # pitch refinement
...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ...
    split = np.zeros(len(ph_list) + 1, int)
    split[0] = 0
    split[-1] = len(mel)
    tg_idx = 0
    ph_idx = 1
    ...
    while tg_idx < len(tg_align):
        ...
        if x["text"] == "sep":
            assert ph == "|", (ph, "|")
        split[ph_idx] = int(
            float(x["xmin"]) * hparams["audio_sample_rate"] / hparams["hop_size"]
        )
        ph_idx += 1
        tg_idx += 1
    assert tg_idx == len(tg_align), ph_idx == len(ph_list)
    split[ph_idx] = int(
        float(x["xmax"]) * hparams["audio_sample_rate"] / hparams["hop_size"]
    )
    ...
    T_t = len(ph_list)
    dur = mel2ph_torch.new_zeros([T_t + 1]).scatter_add(
        0, mel2ph_torch, torch.ones_like(mel2ph_torch)
    )
    dur = dur[1:].numpy()
    return mel2ph, dur
