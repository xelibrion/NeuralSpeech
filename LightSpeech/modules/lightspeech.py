...
from modules.tts_modules import (
    TransformerEncoder,
    LightSpeechDecoder,
    DurationPredictor,
    LengthRegulator,
...
class LightSpeech(nn.Module):
    def __init__(self, arch, dictionary, out_dims=None):
        ...
        self.encoder_embed_tokens = nn.Embedding(
            len(self.dictionary), self.hidden_size, self.padding_idx
        )
        self.encoder = TransformerEncoder(self.enc_arch, self.encoder_embed_tokens)
        self.decoder = (
            LightSpeechDecoder(self.dec_arch) if hparams["dec_layers"] > 0 else None
        )
        self.mel_out = Linear(
            self.hidden_size,
        ...
        if hparams["use_energy_embed"]:
            self.energy_predictor = EnergyPredictor(
                self.hidden_size,
                hparams["energy_predictor_layer"],
                n_chans=hparams["predictor_hidden"],
                dropout_rate=0.5,
                odim=1,
                padding=hparams["ffn_padding"],
    ...
    def forward(
        ...
        # print("mel2ph", mel2ph.shape if mel2ph is not None else "<NONE>")
        # print("mel2ph min", mel2ph.min() if mel2ph is not None else "<NONE>")
        # print("mel2ph max", mel2ph.max() if mel2ph is not None else "<NONE>")
        # if mel2ph is not None:
        #     vals, indices = mel2ph.max(dim=-1)
        #     top_val, idx = vals.max(dim=0)
        # print(vals, top_val, idx)
        # print(mel2ph[idx])
        ...
        if mel2ph is None:
            ...
            ret["mel2ph"] = mel2ph = self.length_regulator(
                dur, (src_tokens != 0).sum(-1)
            )[..., 0]
        else:
        ...
        mel2ph_ = (
            mel2ph.permute([1, 0])[..., None]
            .repeat([1, 1, encoder_out.shape[-1]])
            .contiguous()
        )

        # print("Shapes: decoder_inp:", decoder_inp.shape, "mel2ph_:", mel2ph_.shape)
        # print(
        #     "Values: mel2ph_.min:",
        #     mel2ph_.min().item(),
        #     "mel2ph_.max:",
        ...
        if hparams["use_pitch_embed"]:
            ...
            decoder_inp = decoder_inp + self.add_pitch(
                decoder_inp_origin, pitch, uv, mel2ph, ret
            )
        # add energy embed
        if hparams["use_energy_embed"]:
            with utils.Timer("energy", print_time=hparams["profile_infer"]):
                decoder_inp = decoder_inp + self.add_energy(
                    decoder_inp_origin, energy, ret
                )
    ...
    def add_pitch(self, decoder_inp_origin, pitch, uv, mel2ph, ret):
        ...
        if pitch is not None:  # train
            ...
            pitch_restore = restore_pitch(
                pitch,
                uv if hparams["use_uv"] else None,
                hparams,
            ...
            # with utils.Timer('restore_pitch', print_time=hparams['profile_infer']):
            pitch_restore = restore_pitch(
                pitch, uv, hparams, pitch_padding=pitch_padding
            )
            ret["pitch"] = pitch_restore
            # with utils.Timer('f0_to_coarse', print_time=hparams['profile_infer']):
            pitch_restore = f0_to_coarse_torch(pitch_restore)
